...
### ~~Second CRDT backend~~ — DONE 2026-08-21 (all three libraries)
...
`LoroAdapter` followed the same day (`crates/replica/src/adapter/loro.rs`; `--crdt loro`), completing RQ-1's three libraries. It confirmed the Yrs finding as a pattern rather than a one-off: Loro also ships no per-peer sync protocol (`loro::sync` is a re-export of the crate's own mutex types, not a peer protocol), so it too needs a harness-supplied `HashMap<peer, VersionVector>`. **Two of the three libraries require the benchmark author to invent the sync layer they are then measured through** — that is the RQ-3 methods point, stated more strongly than the Yrs work alone could support.

Loro also forced the conformance suite's universal/characterization line to move in both directions, which is the first evidence the split is doing real work rather than encoding Automerge's habits: three properties became universal (the two `reset` replacements Yrs had hand-written locally, which Loro needed verbatim), and one left (`doc_size_grows_with_every_op` — Loro's snapshot carries a state section and can *shrink* on a delete). Three adapters now give three different answers on save-byte canonicity: Automerge non-canonical by integration order, Yrs canonical, Loro canonical except for random-`PeerID` encoding width (measured 12-16% of trials, flat in history length). A per-replica doc-size spread means something different in each column of the results table.

Deploy-side wiring landed the same day: `--crdt` is plumbed through `gen-compose.py`, the k8s StatefulSet (as a `CRDT` env var), and the `smoke-*`/`bench-*`/`*-up` recipes, which record it in the run-provenance JSON. All three libraries verified end-to-end in docker on `partition-heal-n4` and `divergence-n2-same_region-1e3`. **E2 is unblocked.**

Doing that wiring immediately found two real bugs that no amount of adapter-level review had: the orchestrator's `map_put` sends `obj: ""` (Automerge's ROOT), which both other adapters rejected outright; and `YrsAdapter::sync_generate` never propagated a delete once a peer was caught up, because Yrs deletion advances no clock and the "nothing new" check was a state-vector comparison. The second would have silently failed to converge on any delete-bearing workload. Both are fixed, and `root_map_ops_are_supported` now covers the gap. Process note worth keeping: the conformance suite was written alongside the first adapter and therefore tested the op shapes *that adapter's author* had in mind — it never exercised the empty object name the real caller has always sent. Cross-check a suite against what the callers emit, not only against the trait.
