...
# Shellcheck the bench runner scripts. Separate from `lint` so a machine
# without shellcheck installed still gets a useful `just lint`; `just ci`
# runs both. `-x` follows the `.` into scripts/lib/, which is what lets it
# check the shared functions against their call sites rather than in isolation.
lint-sh:
    #!/usr/bin/env bash
    set -euo pipefail
    if ! command -v shellcheck >/dev/null; then
        echo "shellcheck not installed — skipping (apt-get install shellcheck)" >&2
        exit 0
    fi
...
# Multi-trial docker benchmark: groups scenarios by node_count, brings up a docker stack sized to each group's N and backed by `crdt` (Reset between trials within a group), and concatenates per-group CSVs into results/results-docker.csv for notebook analysis. Single-N callers (e.g. `scenarios/*-n5*.toml`) get one stack up/down; mixed-N callers (e.g. `scenarios/*.toml`) get one stack per distinct N. Pass scenarios as a space-separated string. Exclusive of `just docker-up`/`just docker-down` (owns the stack lifecycle). Implemented in `scripts/bench-docker.sh` over `scripts/lib/bench-common.sh` — read those for the details, including why the run is only trustworthy when the provenance file's `deferred_total` is 0 and `tx_total` is not.
...
# Full CI gate: format check → lint (rust + shell) → test → smoke → scenario-grid check
ci:
    cargo fmt --all --check
    just lint
    just lint-sh
